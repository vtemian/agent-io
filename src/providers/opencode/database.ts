...
export function createOpenCodeDatabase(db: database.Database): OpenCodeDatabase {
  return {
    findProjectIds: (workspacePaths) => findProjectIds(db, workspacePaths),
    findSessions: (projectIds, updatedAfter) => findSessions(db, projectIds, updatedAfter),
    getSessionStats: (sessionIds) => getSessionStats(db, sessionIds),
    getLatestUserSummary: (sessionId) => getLatestUserSummary(db, sessionId),
    getDataVersion: () => getDataVersion(db),
  };
}

function findProjectIds(db: database.Database, workspacePaths: string[]): string[] {
...
function getSessionStats(db: database.Database, sessionIds: string[]): Map<string, SessionStats> {
  if (sessionIds.length === 0) {
    return new Map();
  }

  const messageCounts = queryGroupedCounts(db, "message", "", sessionIds);
  ...
  const assistants = queryLatestAssistants(db, sessionIds);

  return assembleStats(sessionIds, messageCounts, toolCounts, assistants);
}
...
function assembleStats(
  ...
  assistants: Map<string, { agent: string | null; model: string | null }>,
...
function getLatestUserSummary(db: database.Database, sessionId: string): string | undefined {
  const raw = db
    .prepare(
  ...
  return extractSummaryTitle(raw.data);
}

function extractSummaryTitle(jsonData: string): string | undefined {
  try {
...
function getDataVersion(db: database.Database): number {
